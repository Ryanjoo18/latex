...
The \vocab{arc length} of a function $y=f(x)$ in the interval $[a,b]$ is given by
\begin{equation}
...
we aim to find the center of mass of a thin plate with uniform (area) density $\rho$. The \vocab{center of mass} of a region is the point in which the region will be perfectly balanced horizontally if suspended from that point.
...
If $\lim_{n\to\infty}a_n$ exists and is finite we say that the sequence is \vocab{convergent}. If $\lim_{n\to\infty}a_n$ does not exist or is infinite we say the sequence \vocab{diverges}.
